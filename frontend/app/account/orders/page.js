...
// Features: status filter tabs, search by order ID, pagination, expandable order detail.
        ...
        <div className="aom-body">
          ...
          {order.status !== 'cancelled' && (
            <div className="aom-progress">
              {['pending','confirmed','dispatched','delivered'].map((s, i, arr) => {
                ...
                return (
                  <div key={s} className="aom-prog-step">
                    <div className="aom-prog-dot-wrap">
                      {i > 0 && <div className="aom-prog-line" style={{ background: i <= currentIdx ? '#facc15' : '#1a2d4a' }} />}
          ...
          {(order.delivery_address || order.phone || order.notes) && (
            <>
              ...
              <div className="aom-delivery-card">
                ...
                {order.phone && (
                  <div className="aom-drow"><span>📞</span><span>{order.phone}</span></div>
                )}
                {order.notes && (
...
export default function AccountOrdersPage() {
  ...
  useEffect(() => {
    ...
    supabase
      .from('orders')
      .select('id, status, total, created_at, items, delivery_address, phone, notes')
      .eq('user_id', user.id)
      .order('created_at', { ascending: false })
      .then(({ data, error: err }) => {
        if (err) setError('Could not load orders. Please try again.');
        else setOrders(data ?? []);
        setLoading(false);
      });
  }, [user, supabase]);
  ...
  const counts = useMemo(() => {
    ...
    TABS.slice(1).forEach(s => { c[s] = orders.filter(o => o.status === s).length; });
    return c;
  }, [orders]);
  ...
  return (
    <>
      <style>{`
        ...
        .ao-inner { max-width: 860px; margin: 0 auto; }
        ...
        .ao-tab {
          padding: 7px 14px; border-radius: 20px; border: 1px solid #1a2d4a;
        ...
        .ao-search-ico  { position: absolute; left: 13px; top: 50%; transform: translateY(-50%); font-size: 14px; opacity: 0.35; }
        .ao-search {
        ...
        .ao-card-bottom {
          ...
          padding-top: 12px; border-top: 1px solid #0f1a2e;
        }
        ...
        .ao-empty {
          ...
          background: rgba(255,255,255,0.02); border: 1px dashed #1a2d4a;
          border-radius: 16px;
        }
        .ao-empty-icon  { font-size: 48px; margin-bottom: 14px; display: block; opacity: 0.5; }
        ...
        .ao-skel {
          height: 110px; background: linear-gradient(90deg, #0d1b2e 25%, #111f35 50%, #0d1b2e 75%);
          background-size: 200% 100%;
          border-radius: 16px; border: 1px solid #1a2d4a;
          animation: aoSkel 1.4s ease infinite;
        }
        ...
        .ao-page-btn {
          width: 36px; height: 36px; border-radius: 10px; border: 1px solid #1a2d4a;
        ...
        .aom-overlay {
          ...
          display: flex; align-items: center; justify-content: center;
          padding: 16px; animation: aomFade 0.2s ease;
        }
        @keyframes aomFade { from{opacity:0} to{opacity:1} }

        .aom-modal {
          background: #0c1525; border: 1px solid #1a2d4a;
          border-radius: 22px; width: min(540px, 100%);
          max-height: 88vh; overflow-y: auto;
          box-shadow: 0 32px 80px rgba(0,0,0,0.7);
          animation: aomSlide 0.28s cubic-bezier(0.16,1,0.3,1);
        }
        ...
        .aom-item-img {
          ...
          background: #111f35; border: 1px solid #1a2d4a;
        ...
        @media (max-width: 600px) {
          .ao-tabs { gap: 4px; }
          .ao-tab  { padding: 6px 10px; font-size: 11px; }
          .aom-progress { gap: 4px; }
          .aom-prog-label { font-size: 9px; }
        }
      `}</style>
              ...
              <div className="ao-tabs">
                {TABS.map(t => (
                  <button key={t} className={`ao-tab${tab === t ? ' active' : ''}`} onClick={() => setTab(t)}>
                    {t === 'all' ? '🗂️' : STATUS_META[t]?.icon} {t.charAt(0).toUpperCase() + t.slice(1)}
                      ...
                      <div key={order.id} className="ao-card" onClick={() => setSelected(order)}>
                        ...
                        <div className="ao-card-bottom">
                          <div>
                            ...
                            <p className="ao-card-total">{fmtRs(order.total)}</p>
                          </div>
              ...
              {totalPages > 1 && (
                <div className="ao-pagination">
                  ...
                  {Array.from({ length: totalPages }, (_, i) => i + 1).map(p => (
                    <button key={p} className={`ao-page-btn${p === page ? ' active' : ''}`} onClick={() => setPage(p)}>
                      {p}
                    </button>
                  ))}
