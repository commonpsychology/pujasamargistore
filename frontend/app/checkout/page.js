...
export default function Checkout() {
  ...
  // Pre-fill from profile — user can still edit
  ...
  // Once profile loads, fill name + phone (only if fields are still empty)
  useEffect(() => {
    if (profile) {
      if (!name.trim())  setName(profile.full_name  || profile.name  || user?.user_metadata?.full_name || '');
      if (!phone.trim()) setPhone(profile.phone      || profile.phone_number || user?.user_metadata?.phone || '');
    } else if (user && !profile) {
      // Fallback to user_metadata if profile hasn't loaded yet
      if (!name.trim())  setName(user.user_metadata?.full_name || user.user_metadata?.name || '');
      if (!phone.trim()) setPhone(user.user_metadata?.phone || '');
    }
    ...
    try {
      const res = await fetch('/api/orders', {
        ...
        body: JSON.stringify({
          ...
          deliveryCharge,
          grandTotal,
          paymentMethod:   'pending', // updated by payment page
          customerName:    name,
          customerPhone:   phone,
      ...
      // ✅ DO NOT clearCart here — cart is cleared after payment is confirmed
      const params = new URLSearchParams({
        orderId:  data.orderId,
        orderRef: data.orderRef,   // ← PS-XXXXXX-XXXX reference code
        total:    grandTotal,
        name,
        phone,
        address,
              ...
              <div className="form-row">
                <div className="form-group">
                  <label className="form-label">
                    Your Name {name && <span className="autofill-tag">auto-filled</span>}
                  </label>
                ...
                <div className="form-group">
                  <label className="form-label">
                    Phone Number {phone && <span className="autofill-tag">auto-filled</span>}
                  </label>
